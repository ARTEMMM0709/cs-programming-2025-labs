...
# 1. Фильтрация отчетов, содержащих ссылки
reports_with_links = list(filter(
    lambda report: "http://" in report["text"] or "https://" in report["text"],
    reports
))

print("Отчеты, содержащие ссылки:")
for report in reports_with_links:
    print(f"{report['author']}: {report['text']}")

print("\n" + "="*50 + "\n")

# 2. Преобразование отчетов с заменой ссылок
cleaned_reports = list(map(
    lambda report: {
        "author": report["author"],
        "text": report["text"].replace("http://", "[ДАННЫЕ УДАЛЕНЫ]")
                          .replace("https://", "[ДАННЫЕ УДАЛЕНЫ]")
    },
    reports_with_links
))

print("Очищенные отчеты (ссылки заменены):")
